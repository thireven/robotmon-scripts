...
### Added
- "Maximum chain length" setting (default 0 = no limit, so nothing changes unless it is set). Caps the chain the board scan may return, trading a few long chains for many short ones — which is what Roxas- and Maleficent-type Tsums want, and what a high-FPS setup can actually keep up with. (Ported from TsumBeta v84.)
  - It is a stopping condition inside the longest-path search, not a trim applied afterwards: once a chain of that length exists there is nothing better to find, so the DFS returns straight away and the extension pass is skipped. On a 60-tsum board that halved the time `calculatePaths` spends per scan (0.047ms → 0.022ms at a cap of 3), which is the other half of why a cap helps.
  - Verified offline against synthetic boards at caps 0/3/5/8/12: chains never exceed the cap, every returned chain is still contiguous and free of repeated tsums, and a cap above what the board offers returns exactly the same chains as no cap at all.
  - TsumBeta caps its greedy nearest-neighbour walk and has no "off" value (3-15, defaulting to 3, so every chain is 3 long). Here the search is a graph longest-path, and the default has to stay "play the longest chain there is", so 0 was added as the no-limit value and made the default.
  - Click Assist is deliberately exempt — that is the user pointing at a chain and asking for it, so it always draws the whole thing.
- With "Debug logs" on, each board scan also logs the lengths of the chains it found, in play order — what "Maximum chain length" and "Chains per board scan" are tuned against. (TsumBeta v84 logs every path's coordinates; lengths are the part that is actually read, and one line per scan does not bury the rest of the log.)
...
- Re-measured the eight lock-badge samples "Unlock Level every hours" reads on the Tsum collection page: the pale badge behind the padlock is (239, 247, 255) on the current client, where the recorded values were 2-5 counts darker per channel. `isSameColor` allows 20, so this was drift rather than a break — but the page fingerprint beside it had already been re-measured for the same client update, and stale samples are not worth keeping. (Ported from TsumBeta v84, measured there by DRP.)
